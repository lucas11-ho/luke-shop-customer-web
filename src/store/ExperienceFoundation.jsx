...
const RADIUS={small:{control:'8px',card:'12px',media:'10px',sheet:'16px'},medium:{control:'11px',card:'18px',media:'14px',sheet:'22px'},large:{control:'14px',card:'24px',media:'18px',sheet:'28px'},xl:{control:'18px',card:'32px',media:'24px',sheet:'36px'},pill:{control:'999px',card:'24px',media:'18px',sheet:'28px'}};
...
export function ExperienceFoundation(){
 const{experience,effectiveBranding,themePackage}=useStore();
 useEffect(()=>{
  const root=document.documentElement,legacy=experience?.theme||{},packageActive=Boolean(themePackage?.key&&themePackage?.version),foundations=packageActive?(themePackage?.manifest?.foundations||{}):{},packageColors=foundations.colors||{};
  const theme=packageActive?{
   primary:packageColors.primary||legacy.primary,secondary:packageColors.secondary||legacy.secondary,accent:packageColors.accent||legacy.accent,background:packageColors.background||legacy.background,surface:packageColors.surface||legacy.surface,text:packageColors.text||legacy.text,muted_text:packageColors.muted_text||legacy.muted_text,success:packageColors.success||legacy.success,danger:packageColors.danger||legacy.danger,
   radius:foundations.radius||legacy.radius,density:foundations.density||legacy.density,
  }:legacy;
  const radius=RADIUS[theme.radius]||RADIUS.medium,density=DENSITY[theme.density]||DENSITY.comfortable;
 ...
 },[experience,effectiveBranding,themePackage]);
 return null;
}
