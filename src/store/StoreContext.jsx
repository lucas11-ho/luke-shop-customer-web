...
function applyTheme(config={}){
 ...
 const vars={accent:packageColors.primary||theme.primary||branding.accent||DEFAULTS.accent,accent2:packageColors.secondary||theme.secondary||DEFAULTS.accent2,accent3:packageColors.accent||theme.accent||DEFAULTS.accent3,bg:packageColors.background||theme.background||DEFAULTS.bg,surface:packageColors.surface||theme.surface||DEFAULTS.surface,ink:packageColors.text||theme.text||DEFAULTS.ink,muted:packageColors.muted_text||theme.muted_text||DEFAULTS.muted,success:packageColors.success||theme.success||DEFAULTS.success,danger:packageColors.danger||theme.danger||DEFAULTS.danger};
 for(const[k,v]of Object.entries(vars))root.style.setProperty(`--${k}`,COLOR.test(String(v||''))?v:DEFAULTS[k]);
 const radius=themePackage?foundations.radius:theme.radius;root.style.setProperty('--radius',radiusValue(radius));
 const typographyPreset=themePackage?(packageTypography.preset||'SYSTEM_MINIMAL'):typography.preset;const preset=ensureFont(typographyPreset);root.style.setProperty('--font-heading',preset.heading);root.style.setProperty('--font-body',preset.body);
 if(themePackage){const scale=packageScale(packageTypography.scale);root.style.setProperty('--heading-scale',String(scale));root.style.setProperty('--body-scale',String(scale));root.style.setProperty('--letter-spacing','0em');}
 else{root.style.setProperty('--heading-scale',String(scaleHeading(typography.heading_scale)));root.style.setProperty('--body-scale',String(scaleBody(typography.body_scale)));root.style.setProperty('--letter-spacing',typography.letter_spacing==='wide'?'.025em':typography.letter_spacing==='tight'?'-.025em':'0em');}
 const cols=responsive.product_columns||{};root.style.setProperty('--products-desktop',String(cols.desktop||4));root.style.setProperty('--products-tablet',String(cols.tablet||3));root.style.setProperty('--products-mobile',String(cols.mobile||2));root.style.setProperty('--theme-nav-icon-size',`${packageIcons.size||24}px`);
 root.dataset.themePreset=themePackage?`package-${String(themePackage.key||'theme').toLowerCase()}`:(theme.preset||'custom');root.dataset.cardStyle=theme.card_style||'clean';root.dataset.buttonStyle=themePackage?(packageButtons.primary||'solid'):(theme.button_style||'solid');root.dataset.density=themePackage?(foundations.density||'comfortable'):(theme.density||'comfortable');root.dataset.headerLayout=layout.header||'logo_left';root.dataset.heroLayout=layout.hero||'split';root.dataset.categoryLayout=layout.categories||'cards';root.dataset.productGrid=layout.product_grid||'four';root.dataset.productCard=layout.product_card||'standard';root.dataset.mobileNav=themePackage?(packageNavigation.mobile||'standard'):(layout.mobile_nav||'standard');root.dataset.typography=typographyPreset||'SYSTEM_MINIMAL';root.dataset.buttonCase=typography.button_case||'none';root.dataset.heroMediaDesktop=responsive.hero_media_position?.desktop||'right';root.dataset.heroMediaTablet=responsive.hero_media_position?.tablet||'right';root.dataset.heroMediaMobile=responsive.hero_media_position?.mobile||'below';
 root.dataset.themeSystem=themePackage?'v1':'';root.dataset.themePackage=themePackage?`${themePackage.key}@${themePackage.version}`:'';root.dataset.themeRadius=themePackage?(foundations.radius||'medium'):'';root.dataset.themeDensity=themePackage?(foundations.density||'comfortable'):'';root.dataset.themeButtonPrimary=themePackage?(packageButtons.primary||'solid'):'';root.dataset.themeButtonSize=themePackage?(packageButtons.size||'standard'):'';root.dataset.themeNav=themePackage?(packageNavigation.mobile||'standard'):'';root.dataset.themeNavLabels=themePackage?(packageNavigation.labels||'always'):'';root.dataset.themeNavIndicator=themePackage?(packageNavigation.active_indicator||'filled_icon'):'';root.dataset.themeNavContainer=themePackage?(packageNavigation.container||'edge'):'';root.dataset.themeIconActive=themePackage?(packageIcons.active_style||'filled'):'';root.dataset.themeIconInactive=themePackage?(packageIcons.inactive_style||'outline'):'';
...
export function StoreProvider({children}){
 ...
 const value={config,tenant:config?.tenant||null,store:config?.store||null,routing:config?.routing||null,channels:config?.channels||{},experience,themePackage:config?.theme_package||null,effectiveBranding,customerService:config?.customer_service||null,loading,error,errorCode,reload,publicApi:api};
