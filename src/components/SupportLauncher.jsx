...
export function SupportLauncher({placement='floating'}){
 ...
 return <div className={placement==='floating'?'support-floating':'support-inline'}><button className="support-button" onClick={open} disabled={busy} data-testid="support-launcher"><Icon name="headset" size={17}/>{busy?'Connecting…':customerService?.label||'Customer Support'}</button>{message&&<div className="support-note" onClick={()=>setMessage('')}>{message}</div>}</div>;
}
