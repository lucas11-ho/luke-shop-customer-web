...
export function SupportLauncher({placement='floating'}){
 const{customerService}=useStore();const{api,isAuthenticated}=useAuth();const[busy,setBusy]=useState(false);const[message,setMessage]=useState('');
 const enabled=Boolean(customerService?.enabled&&customerService?.placement?.[placement]);if(!enabled)return null;
