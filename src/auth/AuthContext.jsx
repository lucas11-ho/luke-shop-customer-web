...
export function AuthProvider({children}){
  ...
  const login=async({email,password})=>acceptAuth(await api.request('/v1/customer/auth/login',{method:'POST',body:{email,password},auth:false}));
  const register=async({email,password,displayName})=>acceptAuth(await api.request('/v1/customer/auth/register',{method:'POST',body:{email,password,display_name:displayName},auth:false}));
  const loginWithGoogle=async credential=>acceptAuth(await api.request('/v1/customer/auth/google',{method:'POST',body:{credential},auth:false}));
