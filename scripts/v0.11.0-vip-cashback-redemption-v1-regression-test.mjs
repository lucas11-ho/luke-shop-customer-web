...
test('UI copy preserves Backend authority for final redemption amount',()=>{assert.match(checkout,/server confirms the final eligible amount during checkout/i);assert.match(checkout,/Backend rechecks balance, expiry, promotions, delivery and the policy inside the order transaction/);});
